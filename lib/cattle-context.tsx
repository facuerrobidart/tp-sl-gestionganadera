    ...
    const fetchData = async () => {
      try {
        ...
        const transformedCattle: Cattle[] = cattleData.data.map((cow: any) => ({
          ...
          // Transformar de formato GeoJSON a arreglo [lat, lng]
          position:
            cow.position.type === "Point"
              ? [cow.position.coordinates[1], cow.position.coordinates[0]]
              : cow.position,
    ...
    const movementInterval = setInterval(() => {
      setCattle((prevCattle) => {
        return prevCattle.map((cow) => {
          ...
          const isOutside =
            newPosition[0] < minLat ||
            newPosition[0] > maxLat ||
            newPosition[1] < minLng ||
            newPosition[1] > maxLng;
          ...
          if (isOutside) {
            ...
            audio
              .play()
              .catch((e) => console.log("Error reproduciendo alerta:", e));
